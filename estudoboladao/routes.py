...
def editar_viagem(viagem_id):
    ...
    if viagem.autor != current_user:
        flash('Apenas o autor pode editar esta viagem', 'alert-danger')
        return redirect(url_for('usuarios'))
...
@app.route('/viagem/<viagem_id>/excluir', methods=['POST'])
@login_required
def excluir_viagem(viagem_id):
    viagem = Viagem.query.get(viagem_id)
    if viagem.autor != current_user:
        flash('Apenas o autor pode deletar esta viagem', 'alert-danger')
        return redirect(url_for('usuarios'))
